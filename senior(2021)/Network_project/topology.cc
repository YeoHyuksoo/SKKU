...
{
	static double bytes0=0, bytes1=0, bytes2=0;
	if (context == "DefaultFlow"){
		bytes0+=p->GetSize();
...
main (int argc, char *argv[])
{
  ...
  app.Get(0)->TraceConnect("Rx", "Flow1", MakeCallback (&Rxtime));

  // 
  // Create a similar flow from n3 to n0, starting at time 3.0 seconds
  //
  ...
  app2 = sink.Install (terminals.Get (0));
  app2.Start (Seconds (0.0));
  app2.Get(0)->TraceConnect("Rx", "Flow2", MakeCallback (&Rxtime));
  
  // node 2 to node 1 - DefaultFlow
  uint16_t port2 = 19;
  OnOffHelper onoff2 ("ns3::UdpSocketFactory", 
                     Address (InetSocketAddress (Ipv4Address ("10.1.1.2"), port2)));
  onoff2.SetConstantRate (DataRate ("10Mb/s"));
  ApplicationContainer app3 = onoff2.Install (terminals.Get (2));
  app3.Start (Seconds (1.0));
  app3.Stop (Seconds (10.0));
  PacketSinkHelper sink2 ("ns3::UdpSocketFactory",
                        Address (InetSocketAddress (Ipv4Address::GetAny (), port2)));
  app3 = sink2.Install (terminals.Get (1));
  app3.Start (Seconds (0.0));
  app3.Get(0)->TraceConnect("Rx", "DefaultFlow", MakeCallback (&Rxtime));
